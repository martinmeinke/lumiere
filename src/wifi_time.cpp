...
void WiFiEventHandler::handleEvent(esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ESP_LOGI(TAG, "event_base: %s, event_id: %d", event_base, static_cast<int>(event_id));
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START)
    {
        // if scanning for open wifis, we do not want to directly connect after starting the station
    ...
    else if (event_base == IP_EVENT && (event_id == IP_EVENT_STA_GOT_IP || event_id == IP_EVENT_GOT_IP6))
    {
        m_retry_num = 0;
...
void WifiTime::setup_stack()
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT,
                                                        IP_EVENT_STA_GOT_IP,
                                                        &WiFiEventHandler::eventHandlerCallback,
                                                        NULL,
                                                        &instance_got_ip));
...
void WifiTime::connect(const std::string ssid, const std::string pass)
{
    ...
    ESP_LOGI(TAG, "Setting wifi config to [%s/%s]", reinterpret_cast<char *>(wifi_config.sta.ssid), reinterpret_cast<char *>(wifi_config.sta.password));
